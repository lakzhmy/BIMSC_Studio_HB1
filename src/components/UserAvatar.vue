<template>
  <div 
    ...
    :style="{
      width: size,
      height: size,
      animationDuration: `${userStore.avatarConfig.speed}s`,
      background: getBlobGradient(),
      filter: `blur(${userStore.avatarConfig.wobble / 15}px)`,
      opacity: 0.9
    }"
  />
</template>
...
function getComplexityLevel() {
  const complexity = userStore.avatarConfig.complexity
  if (complexity < 25) {
    return 'simple'
...
function getBlobGradient() {
  const shade = userStore.avatarConfig.shade || 2
  const shades = getTeamShades()
  
  const color1 = shades[Math.max(0, shade - 1)]
  const color2 = shades[shade]
  const color3 = shades[Math.min(4, shade + 1)]
