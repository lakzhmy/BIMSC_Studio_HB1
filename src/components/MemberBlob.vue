<template>
  <div 
    ...
    :style="{
      width: size,
      height: size,
      animationDuration: `${member.avatar.speed}s`,
      background: getBlobGradient(),
      filter: `blur(${member.avatar.wobble / 15}px)`,
      opacity: 0.9
    }"
  />
</template>
...
function getComplexityLevel() {
  const complexity = props.member.avatar.complexity
  if (complexity < 25) {
    return 'simple'
...
function getBlobGradient() {
  const shade = props.member.avatar.shade || 2
  const shades = getTeamShades(props.member.teamId)
