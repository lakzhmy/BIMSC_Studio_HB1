    ...
    <main class="py-8 px-6">
      <div class="max-w-7xl mx-auto space-y-6">
        <div class="flex items-center justify-between mb-6">
          <div>
            <h1 class="text-3xl font-bold text-slate-900">Actions</h1>
            <p class="text-slate-600 mt-1">Task management and tracking</p>
          </div>
          <button class="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors font-medium">Create Action</button>
        </div>
        <div class="space-y-4">
          <div v-for="action in actions" :key="action.id" class="bg-white p-6 rounded-lg border border-slate-200 hover:shadow-lg transition-shadow">
            <div class="flex items-start justify-between mb-3">
              <div class="flex-1">
                <h3 class="text-lg font-bold text-slate-900">{{ action.title }}</h3>
                <p class="text-sm text-slate-600 mt-1">{{ action.description }}</p>
              </div>
              <span :class="['px-3 py-1 rounded-full text-xs font-medium', action.priority === 'high' ? 'bg-red-100 text-red-700' : action.priority === 'medium' ? 'bg-amber-100 text-amber-700' : 'bg-green-100 text-green-700']">{{ action.priority }}</span>
            </div>
...
import { actions, projectHealth as projectHealthData } from '@/data/sampleData'
