    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="flex items-center justify-between mb-6">
        <div>
          ...
          <p class="text-slate-500 mt-1">Pop as many blobs as you can in 30 seconds!</p>
        </div>
        <!-- Play As selector -->
      ...
      <!-- Main Grid: Game Area + Leaderboard -->
      <div class="grid lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2">
          <div class="card p-6 flex flex-col gap-4">
            ...
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-6">
                <div class="text-center">
                  <p class="text-xs text-slate-500 font-medium uppercase tracking-wide">Score</p>
                  <p class="text-4xl font-bold text-slate-900">{{ score }}</p>
                </div>
                <div class="text-center">
              ...
              <div class="text-right">
                ...
                <template v-else-if="gameState === 'playing'">
                  <div class="px-4 py-2 bg-green-100 rounded-lg text-green-700 text-sm font-semibold animate-pulse">
                    Go! Pop them!
                  </div>
                </template>
                <template v-else>
                  ...
                  <div class="space-y-2">
                    <p class="text-lg font-bold text-slate-900">
                      Health: <span :class="healthColorClass">{{ lastHealth }}%</span>
                    </p>
                    <p v-if="isNewBest" class="text-xs text-amber-600 font-semibold">✦ New Personal Best!</p>
            ...
            <div
              ...
              style="height: 420px; background: linear-gradient(135deg, #f8fafc 0%, #e2e8f0 100%);"
            >
              <!-- Idle / Done overlay -->
              <transition name="fade">
                <div
                  v-if="gameState !== 'playing'"
                  class="absolute inset-0 flex flex-col items-center justify-center z-10 bg-white/60 backdrop-blur-sm rounded-xl"
                >
                  <template v-if="gameState === 'idle'">
                    <p class="text-2xl font-bold text-slate-700 mb-2">Ready?</p>
                    <p class="text-slate-500 text-sm">Click "Start Game" to begin</p>
                  </template>
                  <template v-else>
                    <p class="text-2xl font-bold text-slate-700 mb-1">Time's Up!</p>
                    <p class="text-5xl font-black text-slate-900 mb-2">{{ score }}</p>
                    <p class="text-slate-500 text-sm">blobs popped → <span class="font-semibold" :class="healthColorClass">{{ lastHealth }}% health</span></p>
                  </template>
                </div>
              </transition>
        ...
        <div class="space-y-4">
          <!-- Team Health Summary -->
          <div class="card p-5">
            <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">Team Health</h2>
            <div class="space-y-4">
              <div v-for="team in teamHealthSummary" :key="team.id">
                <div class="flex items-center justify-between mb-1">
                  ...
                  <span class="text-sm font-bold text-slate-900">{{ team.health }}%</span>
                </div>
          ...
          <!-- Member Leaderboard -->
          <div class="card p-5">
            <h2 class="text-sm font-semibold text-slate-700 uppercase tracking-wide mb-4">Leaderboard</h2>
            <div class="space-y-3">
              <div
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="h-1 bg-slate-200 rounded-full mt-1">
                    <div
                      ...
                      :class="member.teamId === 'structure' ? 'bg-green-500' : member.teamId === 'program' ? 'bg-blue-500' : 'bg-red-500'"
...
// ── All members (same logic as DashboardView) ──────────────────────────────
...
// Default to the first member of the user's own team
...
// Initialise lazily so the computed is ready
...
const allGradients = Object.values(TEAM_GRADIENTS).flat()
...
function spawnBlob() {
  ...
  const morphClass = BLOB_MORPH_CLASSES[Math.floor(Math.random() * BLOB_MORPH_CLASSES.length)]
...
function popBlob(id) {
  ...
  score.value++
  // Immediately spawn a replacement so density stays high
  spawnBlob()
}
...
function startGame() {
  ...
  // Periodic re-spawn to keep density
...
function endGame() {
  ...
  const health = Math.min(100, Math.round((score.value / 60) * 100))
...
const healthColorClass = computed(() => {
  if (lastHealth.value >= 70) return 'text-green-600'
  if (lastHealth.value >= 40) return 'text-amber-600'
  return 'text-red-600'
})
...
const teamHealthSummary = computed(() => {
  const config = [
    { id: 'structure', name: 'Green Structure', textClass: 'text-green-700', barClass: 'bg-green-500' },
    { id: 'program', name: 'Blue Program', textClass: 'text-blue-700', barClass: 'bg-blue-500' },
    { id: 'data', name: 'Red Data', textClass: 'text-red-700', barClass: 'bg-red-500' },
  ]
  return config.map((t) => ({
    ...t,
    health: userStore.getTeamHealth(t.id),
  }))
})
</script>
...
/* Blob morphing (reused from MemberBlob) */
.blob-morphing {
  border-radius: 50%;
