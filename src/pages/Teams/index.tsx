import { Construction } from 'lucide-react';

/**
 * Teams Page
 *
 * Owner: Data Team
 *
 * TODO: Build this page to show:
 * - Team cards with members
 * - Team-specific KPI summary
 * - Team activity feed
 * - Member profiles
 */
export function TeamsPage() {
  return (
    <div className="p-6 bg-slate-50 min-h-screen flex items-center justify-center">
      <div className="text-center max-w-md">
        <Construction size={64} className="text-slate-300 mx-auto mb-4" />
        <h1 className="text-2xl font-bold text-slate-900 mb-2">Teams</h1>
        <p className="text-slate-500 mb-4">
          Manage team members and view team-specific dashboards.
        </p>
        <div className="bg-white rounded-lg border border-slate-200 p-4 text-left">
          <p className="text-sm font-medium text-slate-700 mb-2">Features to build:</p>
          <ul className="text-sm text-slate-500 space-y-1">
            <li>• Team overview cards</li>
            <li>• Member list with avatars</li>
            <li>• Team-specific KPI summary</li>
            <li>• Recent activity feed per team</li>
            <li>• Team workload visualization</li>
          </ul>
        </div>
        <p className="mt-4 text-sm text-slate-400">
          Design this page in Figma, then use Claude to build it!
        </p>
      </div>
    </div>
  );
}
